...
const rentalController = {
    ...
    createRental: async (req, res) => {
      try {
        const { pickupLocation, dropoffLocation, pickupTime, dropoffTime } = req.body;
        ...
        if (pickupDate >= dropoffDate) {
          return res.status(400).json({ message: 'Drop-off time must be after pickup time' });
        }
        ...
        res.status(500).json({ success: false, message: error.message || 'Error creating rental' });
      }
    },
    ...
    getAllRentals: async (req, res) => {
      try {
        ...
        res.status(500).json({ success: false, message: error.message || 'Error fetching rentals' });
      }
    },
    ...
    getRentalById: async (req, res) => {
      try {
        ...
        res.status(500).json({ success: false, message: error.message || 'Error fetching rental' });
      }
    },
    ...
    updateRental: async (req, res) => {
      try {
        if (req.body.pickupTime && req.body.dropoffTime) {
          ...
          if (pickupDate >= dropoffDate) {
            return res.status(400).json({ success: false, message: 'Drop-off time must be after pickup time' });
          }
        }
        ...
        res.status(500).json({ success: false, message: error.message || 'Error updating rental' });
      }
    },
    ...
    deleteRental: async (req, res) => {
      try {
        ...
        res.status(500).json({ success: false, message: error.message || 'Error deleting rental' });
      }
    }
  };
